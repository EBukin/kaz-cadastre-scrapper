...
# 
# 
# write_rds(harvested_raw, here("data-raw", "18-plot-metadata-all.rds"))
...
parse_metadata <- function(rwjson) {
  ...
  raw_json <- rwjson %>% 
    # harvested_raw$raw_mtdt_responce %>% 
    # sample(1) %>% 
  ...
  #       str_replace_all(" ", "_") %>%
  #       str_to_lower(),
  #     kaz = Kz %>% translit_kaz_text,
  #     rus = Ru %>% translit_rus_text 
  #   ) %>% 
