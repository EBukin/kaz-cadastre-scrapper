...
### 2.1 Harveting kvartals index in the rayons of interest
...
1. Identify ZZZ for this kvartal.
...
### 3.1 Fishin first digit of the plot ID in regions of interest

When we have three digits of each kvartal, we can start getting IDs of plots. 
...
### 3.2 Downloading 3 digit codes for each plot

separate script

### 3.3 Getting additional plots with 4 digits which were ommited at first run

Separate script.

### 3.4 Cleaning plot-level data and saving it clean
...
In kegenskiy rayon there are plots which have IDs that do not contain number of the rayon... For example id "0305008100" in rayon 323

[http://www.aisgzk.kz/aisgzk/Proxy/aisgzkZem2/MapServer/find?searchText=0305008100&contains=true&searchFields=KAD_NOMER&layers=75&returnGeometry=true&f=pjson](http://www.aisgzk.kz/aisgzk/Proxy/aisgzkZem2/MapServer/find?searchText=0305008100&contains=true&searchFields=KAD_NOMER&layers=75&returnGeometry=true&f=pjson)
...
### 4.1 Extracting metadata for existing cadatre codes



### 4.2 Cleaning/saving metadata for existing cadastre codes
