...
```{r}
ugod_shps <- read_rds(
  here("data-clean", "10-sarah-request", "kaz-akmol-3-ray-ugodia-types.rds"))
...
Data below is present for 3 rayons.
...
ugod_sum %>%
  ...
  group_by(raj_rus) %>% 
  mutate(area_ray_2 =  sum(`% share of rayon land` / 100)  %>% scales::percent(0.1)) %>% 
